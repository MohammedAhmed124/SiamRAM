...
The submission's `requirements.txt` targets x86 CUDA 12.8 and will not work on
Jetson. Use `pyproject.nano.toml`, which skips torch/torchvision/torch-tensorrt/opencv
(provided above / by the system), excludes `triton` (no ARM64 wheel exists), and builds
...
`inference.py` downloads these automatically on its first run, but you can fetch
them ahead of time so the first inference starts offline:
...
- `model.pth` — SiamABC weights (required for inference)
- `yolo11n.pt` — YOLO re-detection weights (required for inference)
- `osnet_x0_25_imagenet.pth` — OSNet appearance descriptor (required for inference)
...
path. The manifest lists each sequence's `video_path` and `annotation_path` (the
first-frame box), read relative to the directory you launch from — so run the
command from the folder that holds the dataset:
...
`inference.py` takes three positional arguments:

```bash
python inference.py <manifest.json> <split> <output.csv>
```

| Position | Example | Description |
|---|---|---|
| 1 | `test.json` | Manifest JSON listing each sequence's `video_path` / `annotation_path` |
| 2 | `public_lb` | The split name (top-level key) to run inside the manifest |
| 3 | `submission.csv` | Output predictions CSV (`id,x,y,w,h`) |

Every other setting (checkpoint paths, model/tracker hyperparameters, the
`use_nvdec` / `cudnn_benchmark` runtime switches) comes from
`src/config/inference_config_944f2b8.yaml`, not from the CLI.
