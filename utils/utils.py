...
def get_extended_crop(image, bbox, crop_size, context, padding_value=None):
    ...
    # ---------------------------------------------------------------
    # BUG FIX: make the crop contiguous BEFORE any cv2 call.
    #
    # The slice above is a non-contiguous VIEW of `image`.  Its row
    # stride equals the FULL source frame width, not the crop width.
    # At 4K (stride = 11 520 B/row) a 500 px-wide crop wastes ~10 KB
    # of cache-line reads per row inside every cv2 function.
    #
    # The original code only applied np.ascontiguousarray in the
    # `else` branch (no-padding path).  When padding IS needed,
    # cv2.copyMakeBorder received the strided view directly — the
    # exact same cache-thrash problem, fully reproducible whenever
    # the tracked object is near any frame edge.
    #
    # Fix: hoist the contiguity check unconditionally so BOTH
    # cv2.copyMakeBorder and cv2.resize always see packed memory.
    # Cost: one memcpy of the crop region (not the whole frame).
    # ---------------------------------------------------------------
